...
Class Master extends DBConnection
{
    ...
    public function save_user(){
        ...
        if(empty($id)){
            ...
            if($qry){
                $this->settings->set_flashdata('success','Client details successfully saved.');
                foreach($_POST as $k => $v){
                    if($k != 'id'){
                        if(!empty($data)) $data .=" , ";
            ...
            if($qry){
                ...
                return "UPDATE clients set $data where id = {$id}";
            }
        }
    }
...
//        $data = "";
//        foreach($_POST as $k =>$v){
//            if(!in_array($k,array('id', 'firstname'))){
//                if(!empty($data)) $data .=",";
//                $data .= " `{$k}`='{$v}' ";
//            }
//        }
//        $check = $this->conn->query("SELECT * FROM `clients` where `lastname` = '{$lastname}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
//        if ($this->capture_err())
//            return $this->capture_err();
//        if ($check > 0) {
//            $resp['status'] = 'failed';
//            $resp['msg'] = "Client already exists";
//            return json_encode($resp);
//        }
//        if (empty($id)) {
